 ...
 *[  𓍢ִ໋💜⊹ ] Si consideras que esto es un error y cuentas con pruebas, puedes comunicarte con el propietario(a) del bot para apelar la suspensión.*`.trim();
              m.reply(messageText);
          ...
          if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
            if (user.commandCount === 2) {
              ...
              if (remainingTime > 0) {
                const messageText = `*[  𓍢ִ໋💜⊹ ] Espera* _${remainingTime} segundos_ *antes de utilizar otro comando.*`;
                m.reply(messageText);
                return;
              } else {
                user.commandCount = 0;
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          mconn.conn.reply(m.chat, `*[  𓍢ִ໋💜⊹ ] Sus diamantes se han agotado, puede adquirir más con el comando:* _${usedPrefix}buyall_`, m);
          continue; 
        }
        if (plugin.level > _user.level) {
          mconn.conn.reply(m.chat, `*[  𓍢ִ໋💜⊹ ] Se require tener el nivel ${plugin.level} para poder utilizar el comando. Tú nivel actual es ${_user.level}, usa el comando ${usedPrefix}lvl para subir tu nivel con XP.*`, m);
          continue; 
        }
        const extra = {
          match,
        ...
        try {
          ...
          if (m.limit) {
            m.reply('*[  𓍢ִ໋💜⊹ ] Se utilizaron ' + +m.limit + ' diamante(s) (limites).*');
          }
        }
        break;
      }
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:; 𓍢ִ໋💜⊹ 𝗩𝗜𝗢𝗟𝗘𝗧 𝗕𝗢𝗧  𓍢ִ໋💜⊹ ;;;\nFN: 𓍢ִ໋💜⊹ 𝗩𝗜𝗢𝗟𝗘𝗧 𝗕𝗢𝗧  𓍢ִ໋💜⊹ \nORG: 𓍢ִ໋💜⊹ 𝗩𝗜𝗢𝗟𝗘𝗧 𝗕𝗢𝗧  𓍢ִ໋💜⊹ \nTITLE:\nitem1.TEL;waid=51906528530:+51 906 528 530\nitem1.X-ABLabel: 𓍢ִ໋💜⊹ 𝗩𝗜𝗢𝗟𝗘𝗧 𝗕𝗢𝗧  𓍢ִ໋💜⊹ \nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME: 𓍢ִ໋💜⊹ 𝗩𝗜𝗢𝗟𝗘𝗧 𝗕𝗢𝗧  𓍢ִ໋💜⊹ \nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: ' 𓍢ִ໋💜⊹ 𝗩𝗜𝗢𝗟𝗘𝗧 𝗕𝗢𝗧  𓍢ִ໋💜⊹ ', contacts: [{vcard}]}}, {quoted: callmsg});
 ...
 *[  𓍢ִ໋💜⊹ ] Para desactivar la función* _antidelete_*, envia el siguiente comando:* _/disable antidelete_`.trim();
...
global.dfail = (type, m, conn) => {
  const msg = {
    rowner: '*[   𓍢ִ໋💜⊹ ] Ey Paraaaa Pndjx Este Comando Solo Lo Usan Los Owners*',
    owner: '*[  𓍢ִ໋💜⊹ ] Paraaa Este Comando Solo Puede Ser Usado Por El Mas Guapo Osea Yo*',
    mods: '*[  𓍢ִ໋💜⊹ ] Este Comando Solo Puede Ser Usado Por Admins Y El Bot.*',
    premium: '*[  𓍢ִ໋💜⊹ ] Si No Eres Usuario Vip Olvidate De Este Comando XD.*',
    group: '*[  𓍢ִ໋💜⊹ ] Solo Apto Para Grupps Mij@*',
    private: '*[  𓍢ִ໋💜⊹ ] Este Comando Solo Se Puede Usar En El Chat Privadp Del Bot*',
    admin: '*[  𓍢ִ໋💜⊹ ] Solo Puede Ser Usado Por Admins Bldx*',
    botAdmin: '*[  𓍢ִ໋💜⊹ ] Es Necesario Que El Mas Guapo Osea Yo Tenga Admin*',
    unreg: '*[  𓍢ִ໋💜⊹ ] Debes Estar Registrado Primerx*\n\n*[  𓍢ִ໋💜⊹ ] Utiliza el comando:* _/verificar nombre.edad_ *para registrarte.*',
